...
**Current Status**: Iter-17/18 in flight. Recent: file save/load with FS Access API + recents (iter-15), NFA→DFA conversion (iter-16), batch input testing modal, image export, command-bar pill, onboarding, keyboard-scope stack, Tier-1 hotkeys (space/arrows/F/?), in-progress transition previews (add/modify/delete), council-driven audit cycles (audit-001 → audit-003). Per-iteration plan + completion notes live in `docs/iterations/`.
